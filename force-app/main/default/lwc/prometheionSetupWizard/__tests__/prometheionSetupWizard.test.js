/**
 * Jest tests for prometheionSetupWizard LWC component
 *
 * Tests cover:
 * - Component rendering
 * - Wizard navigation
...
import PrometheionSetupWizard from "c/prometheionSetupWizard";
...
describe("c-prometheion-setup-wizard", () => {
  afterEach(() => {
  ...
  async function createComponent() {
    const element = createElement("c-prometheion-setup-wizard", {
      is: PrometheionSetupWizard,
    });
  ...
  describe("Initial Rendering", () => {
    it("renders the component", async () => {
      ...
      expect(card.title).toBe("Prometheion Setup Wizard");
    });
    ...
    it("starts on step 1 (Welcome)", async () => {
      ...
      expect(heading.textContent).toContain("Welcome to Prometheion");
    });
  ...
  describe("Accessibility", () => {
    ...
    it("main content has role and aria-label", async () => {
      ...
      expect(main.getAttribute("aria-label")).toBe("Prometheion Setup Wizard");
    });
  });
});
